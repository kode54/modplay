...
static void MainPlayer(PLAYER *p) // periodically called from mixer
{
    ...
    for (i = 0; i < p->Song.AntChn; ++i)
    {
        ...
        if (ch->Status & IS_NyTon)
        {
            ...
            voiceSetVolume(p, ch->Nr + 127, 0, ch->FinalPan);
...
void voiceSetVolume(PLAYER *p, uint8_t i, float vol, uint8_t pan)
{
#ifdef USE_VOL_RAMP
    p->voice[i].targetVolL = vol * p->PanningTab[256 - pan];
    p->voice[i].targetVolR = vol * p->PanningTab[      pan];
    p->voice[i].volDeltaL  = (p->voice[i].targetVolL - p->voice[i].volumeL) / p->f_samplesPerFrame;
    p->voice[i].volDeltaR  = (p->voice[i].targetVolR - p->voice[i].volumeR) / p->f_samplesPerFrame;
#else
...
static void mixSampleBlock(PLAYER *p, int32_t *outputStream, uint32_t sampleBlockLength)
{
...
void ft2play_RenderFixed(void *_p, int32_t *buffer, int32_t count)
{
    PLAYER * p = (PLAYER *)_p;
    int32_t samplesTodo;
...
void ft2play_Render16(void *_p, int16_t *buffer, int32_t count)
{
    ...
    while (count)
    {
        ...
        ft2play_RenderFixed(_p, bufferFixed, samplesTodo);
        count -= samplesTodo;
        samplesTodo <<= 1;
...
#ifdef USE_VOL_RAMP
    p->f_samplesPerFrame = (float)(val) / 4.0f;
#endif
}
